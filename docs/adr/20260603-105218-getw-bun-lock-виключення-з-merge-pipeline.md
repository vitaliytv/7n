...
Уточнення логіки `regen_bun`: прапорець `regen_bun=1` виставляється лише коли `package.json` потрапляє у складний merge-шлях (Tier 1+), а не в момент, коли `bun.lock` виявлено в дельті. `bun.lock` пропускається тихо — без `echo` і без виставлення `regen_bun`. Логіка запуску `bun install` наприкінці не змінюється: він спрацьовує лише якщо `regen_bun=1`.

Конкретна зміна у `npm/getw.js` (~рядки 188-194):
- До: `bun.lock` у дельті → `regen_bun=1` + `echo` + `continue`
- Після: `bun.lock` → тихий `continue`; `package.json` у merge-шляху → `regen_bun=1`
